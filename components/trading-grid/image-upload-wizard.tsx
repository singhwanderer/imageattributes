...
// --- AI metadata suggestion (demo) ---------------------------------------
// Manual flow is the floor: if /api/suggest-metadata is unavailable (missing
// or expired GEMINI_API_KEY, quota, network), the wizard behaves exactly as
// it did before this change. AI is progressive enhancement only.

type ExtendedSuggestion = { attribute: string; value: string; code: string | null; matched?: boolean; confidence: number }
type AiSuggestionResult = {
  fields: Partial<{ orientation: string; facing: string; angle: string; imageStyle: string; imageDescription: string; height: string; width: string }>
  extended: ExtendedSuggestion[]
}
type AiState = "idle" | "loading" | "done" | "unavailable"

const AI_SUGGESTIBLE_FIELDS = ["orientation", "facing", "angle", "imageStyle", "imageDescription", "height", "width"] as const
...
function StepTwoForm({ currentAttrs, updateAttrs, advancedOpen, setAdvancedOpen, uploadLevel, autoData, suggestedFields = [] }: StepTwoFormProps) {
  const isAi = (field: string) => suggestedFields.includes(field)
  return (
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        ...
        <CollapsibleContent>
          <div className="mt-4 grid gap-4 rounded border border-border bg-background p-4 md:grid-cols-2">
            <div className="flex flex-col gap-2">
              <Label className="text-sm font-medium">Image Style {isAi("imageStyle") && <AiBadge />}</Label>
            ...
            <div className="flex flex-col gap-2">
              <Label className="text-sm font-medium">Facing (GDSN) {isAi("facing") && <AiBadge />}</Label>
            ...
            <div className="flex flex-col gap-2">
              <Label className="text-sm font-medium">Angle {isAi("angle") && <AiBadge />}</Label>
            ...
            <div className="flex flex-col gap-2 md:col-span-2">
              <Label className="text-sm font-medium">Image Description {isAi("imageDescription") && <AiBadge />}</Label>
            ...
            <div className="flex flex-col gap-2">
              <Label className="text-xs text-muted-foreground">Height (px) {isAi("height") && <AiBadge />}</Label>
            ...
            <div className="flex flex-col gap-2">
              <Label className="text-xs text-muted-foreground">Width (px) {isAi("width") && <AiBadge />}</Label>
...
export function ImageUploadWizard({
  ...
  // --- AI metadata suggestion state (demo) ---
  const [aiState, setAiState] = useState<AiState>("idle")
  const [aiMarksByImage, setAiMarksByImage] = useState<{ [idx: number]: string[] }>({})
  const [aiExtendedByImage, setAiExtendedByImage] = useState<{ [idx: number]: ExtendedSuggestion[] }>({})
  const [aiRequestedImages, setAiRequestedImages] = useState<Set<number>>(new Set())

  // Object URL -> base64 + natural dimensions (deterministic, client-side).
  const readImage = async (previewUrl: string): Promise<{ base64: string; mimeType: string; width: number; height: number }> => {
    ...
    return { base64, mimeType: blob.type || "image/jpeg", ...dims }
  }

  const requestAiSuggestions = useCallback(async (idx: number) => {
    const file = uploadedFiles[idx]
    if (!file?.preview || aiRequestedImages.has(idx)) return
    setAiRequestedImages((prev) => new Set(prev).add(idx))
    setAiState("loading")
    try {
      const { base64, mimeType, width, height } = await readImage(file.preview)
      const res = await fetch("/api/suggest-metadata", {
        ...
        body: JSON.stringify({
          ...
          enums: {
            ...
            angle: ANGLE_OPTIONS,
            imageStyle: IMAGE_STYLE_OPTIONS,
          },
        }),
      })
      if (!res.ok) { setAiState("unavailable"); return }
      const data = await res.json()
      if (!data.available) { setAiState("unavailable"); return }

      const s = data.suggestions
      const candidate: Record<string, string> = {}
      if (s.orientation) candidate.orientation = s.orientation
      if (s.facing) candidate.facing = s.facing
      if (s.angle) candidate.angle = s.angle
      if (s.imageStyle) candidate.imageStyle = s.imageStyle
      if (s.imageDescription) candidate.imageDescription = s.imageDescription
      if (width > 0) { candidate.width = String(width); candidate.height = String(height) }

      // Merge into empty fields only — never overwrite what the user typed.
      const applyMerge = (current: typeof attributes) => {
        const next = { ...current }
        const applied: string[] = []
        for (const [field, value] of Object.entries(candidate)) {
          if (!next[field as keyof typeof attributes]) {
            next[field as keyof typeof attributes] = value
            applied.push(field)
          }
        }
        return { next, applied }
      ...
      if (applyToAll) {
        const { next, applied } = applyMerge(attributes)
        setAttributes(next)
        setAiMarksByImage((prev) => ({ ...prev, [0]: applied }))
        setAiExtendedByImage((prev) => ({ ...prev, [0]: s.extended ?? [] }))
      } else {
        const { next, applied } = applyMerge(attributesByImage[idx] || attributes)
        setAttributesByImage((prev) => ({ ...prev, [idx]: next }))
        setAiMarksByImage((prev) => ({ ...prev, [idx]: applied }))
        setAiExtendedByImage((prev) => ({ ...prev, [idx]: s.extended ?? [] }))
      }
      setAiState("done")
    } catch {
  ...
  }, [uploadedFiles, aiRequestedImages, applyToAll, attributes, attributesByImage])

  // Fire suggestions on entering Step 2, and on switching image in per-image mode.
  useEffect(() => {
    if (currentStep !== 2 || uploadedFiles.length === 0 || aiState === "unavailable") return
    requestAiSuggestions(applyToAll ? 0 : activeAttributeImageIndex)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentStep, applyToAll, activeAttributeImageIndex, uploadedFiles.length])

  const activeAiIndex = applyToAll ? 0 : activeAttributeImageIndex
  const currentAiMarks = aiMarksByImage[activeAiIndex] || []
  const currentAiExtended = aiExtendedByImage[activeAiIndex] || []
  ...
  const updateCurrentAttributes = (newAttrs: typeof attributes) => {
    // User edit clears the AI-suggested mark for any field whose value changed.
    const current = getCurrentAttributes()
    const changed = Object.keys(newAttrs).filter(
      (k) => newAttrs[k as keyof typeof attributes] !== current[k as keyof typeof attributes],
    )
    if (changed.length > 0) {
      setAiMarksByImage((prev) => ({
        ...prev,
        [activeAiIndex]: (prev[activeAiIndex] || []).filter((f) => !changed.includes(f)),
      }))
    }
    if (applyToAll) {
      ...
      <div className="rounded border border-border bg-card p-6">
        ...
        {currentStep === 2 && (
          <div className="flex flex-col gap-6">
            ...
            {/* AI suggestion status strip — silent when idle; manual flow never blocked */}
            {aiState === "loading" && (
              <div className="flex items-center gap-2 rounded border border-primary/20 bg-primary/5 px-3 py-2 text-xs text-primary">
                <Loader2 className="size-3.5 animate-spin" /> Analyzing image for suggested attributes…
              </div>
            )}
            {aiState === "done" && currentAiMarks.length > 0 && (
              <div className="flex items-center gap-2 rounded border border-primary/20 bg-primary/5 px-3 py-2 text-xs text-primary">
                <Sparkles className="size-3.5" /> AI suggestions applied to {currentAiMarks.length} field{currentAiMarks.length !== 1 ? "s" : ""} — review and edit before proceeding.
              </div>
            )}
            {aiState === "unavailable" && (
              <p className="text-xs text-muted-foreground">AI suggestions unavailable — continue with manual entry.</p>
            )}
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 flex flex-col gap-4">
                  ...
                  <StepTwoForm
                    ...
                    uploadLevel={uploadLevel}
                    autoData={getAutoPopulatedData()}
                    suggestedFields={currentAiMarks}
                  />
                </div>
              </div>
            ) : (
            ...
            {/* Extended product attributes (AI suggested, demo scope) */}
            {currentAiExtended.length > 0 && (
              <div className="flex flex-col gap-3 rounded border border-border bg-muted/20 p-4">
                <div className="flex items-center gap-2">
                  <Sparkles className="size-4 text-primary" />
                  <span className="text-sm font-medium text-foreground">Extended attributes (AI suggested)</span>
                  <span className="text-xs text-muted-foreground">Detected from the image — edit or remove before submitting.</span>
                </div>
                <div className="grid gap-3 md:grid-cols-2">
                  {currentAiExtended.map((ext, i) => (
                    <div key={`${ext.attribute}-${i}`} className="flex items-end gap-2">
                      <div className="flex flex-1 flex-col gap-1">
                        <Label className="text-xs text-muted-foreground">{ext.attribute}
                          <span className="ml-1.5 rounded-full bg-primary/10 px-1.5 py-0.5 text-[10px] font-medium text-primary">
                            {Math.round(ext.confidence * 100)}%
                          </span>
                          {ext.code && (
                            <span className="ml-1.5 font-mono text-[10px] text-muted-foreground">{ext.code}</span>
                          )}
                        </Label>
                        <Input
                          value={ext.value}
                          onChange={(e) => {
                            const next = [...currentAiExtended]
                            next[i] = { ...next[i], value: e.target.value }
                            setAiExtendedByImage((prev) => ({ ...prev, [activeAiIndex]: next }))
                          }}
                          className="bg-background"
                        />
                      </div>
                      <Button
                        ...
                        onClick={() => {
                          setAiExtendedByImage((prev) => ({
                            ...prev,
                            [activeAiIndex]: currentAiExtended.filter((_, j) => j !== i),
                          }))
                        }}
                      >
                        <Trash2 className="size-4" />
